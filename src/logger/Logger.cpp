...
void Logger::out(const std::string &msg, LogLevel LogLevel) {
  ...
  std::strftime(std::data(timeString), std::size(timeString), "%d/%m/%y %T",
                std::gmtime(&time));
  std::cout << timeString << " " << prefix << " " << msg << std::endl;
}
...
void Logger::setLogLevel(LogLevel level) { Logger::log_level = level; }
